...
void Manifold::UpdateConstraint(Contact& contact)
{
	...
	*/
	...
	if (Vector3::Dot(tangent1, tangent1) < 0.001f)
	{
		...
		if (Vector3::Dot(tangent1, tangent1) < 0.001f)
		{
			tangent1 = Vector3::Cross(contact.collisionNormal,
				Vector3(0, 0, 1));
		}
	}
	tangent1.Normalise();
	...
	// Elasticity
	{
		...
		float elatisity_term =
			elasticity * Vector3::Dot(contact.collisionNormal, -m_NodeA->GetLinearVelocity() - Vector3::Cross(r1, m_NodeA->GetAngularVelocity()) + m_NodeB->GetLinearVelocity()
	...
	contact.normal = Constraint(m_NodeA, m_NodeB, -contact.collisionNormal,
		Vector3::Cross(-r1, contact.collisionNormal), contact.collisionNormal,
		Vector3::Cross(r2, contact.collisionNormal), b);

	contact.normal.impulseSumMin = 0.0f;

	contact.friction1 = Constraint(m_NodeA, m_NodeB, -tangent1 * friction,
		Vector3::Cross(-r1, tangent1) * friction, tangent1 * friction,
		Vector3::Cross(r2, tangent1) * friction, 0.0f);

	contact.friction2 = Constraint(m_NodeA, m_NodeB, -tangent2 * friction,
		Vector3::Cross(-r1, tangent2) * friction, tangent2 * friction,
		Vector3::Cross(r2, tangent2) * friction, 0.0f);
}
