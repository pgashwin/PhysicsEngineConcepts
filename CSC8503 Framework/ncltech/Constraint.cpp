...
void Constraint::ApplyImpulse()
{
	...
	*/
	...
	delta = 0.0f;  //	J*M	(-1)*J(t)
	float contstraint_mass = objA->GetInverseMass() * Vector3::Dot(j1, j1) + Vector3::Dot(j2, (objA->GetInverseInertia() * j2)) + objB->GetInverseMass() * Vector3::Dot(j3, j3) + Vector3::Dot(j4, (objB->GetInverseInertia() * j4)) + softness;
	if(contstraint_mass  > 0.00001f)
	{
		//JV
		float jv = Vector3::Dot(j1, objA->GetLinearVelocity()) + Vector3::Dot(j2, objA->GetAngularVelocity()) + Vector3::Dot(j3, objB->GetLinearVelocity()) + Vector3::Dot(j4, objB->GetAngularVelocity());
		float denom = -(jv + b);
