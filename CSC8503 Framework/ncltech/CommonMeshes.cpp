...
Mesh* CommonMeshes::m_tardis    = NULL;
Mesh* CommonMeshes::m_texturedSpehere = NULL;
Mesh* CommonMeshes::m_pyramid = NULL;
...
void CommonMeshes::InitializeMeshes()
{
	...
	if (m_pPlane == NULL)
	{
		...
		m_pyramid = new OBJMesh(MESHDIR"pyramid.obj");

		m_texturedSpehere = new OBJMesh(MESHDIR"texturedsphere.obj");

		m_pPlane->SetTexture(m_CheckerboardTex);
		m_pCube->SetTexture(m_CheckerboardTex);
		m_pSphere->SetTexture(m_CheckerboardTex);
		m_pyramid->SetTexture(m_CheckerboardTex);
	}
}
