...
//	**/
//		for
//			(PhysicsObject* obj : m_PhysicsObjects)
//		{
//			UpdatePhysicsObject(obj);
//		}

void PhysicsEngine::UpdatePhysicsObject(PhysicsObject* obj){//Semi-Implicit	Euler Intergration

	if (obj->restState() == false){
		obj->m_LinearVelocity += obj->m_Force * obj->m_InvMass* m_UpdateTimestep;
		if (obj->m_InvMass > 0.00001f) obj->m_LinearVelocity += m_Gravity * m_UpdateTimestep; //Technically this is(m_Gravity/invMass)*invMass*dt,
		...
		obj->m_Position += obj->m_LinearVelocity * m_UpdateTimestep; //	Angular	Rotation
			...
			if (CollisionDetection::Instance()->CheckCollision(cp.objectA, cp.objectB, shapeA, shapeB, &coldata))
			{
				...
				if (okA && okB)
				{
					...
					if (CollisionDetection::Instance()->BuildCollisionManifold(cp.objectA, cp.objectB, shapeA, shapeB, coldata, manifold))
					{
						...
						//rest state
						cp.objectA->SettoRest(false);
						cp.objectB->SettoRest(false);
